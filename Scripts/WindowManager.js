...
WindowManager.prototype.loadState = function (dialog) {
  // TODO: Load the state from localstorage on object creation, then keep that in memory for reading and add a func like this that takes one dialog as param and only restores for that
  ...
  if (canSave)
    try {
      ...
      if (dialog && dialog.id)
        dialog.loadWindowState(windowStates[dialog.id]), updateTopZ(dialog.z);
      else {
        var fails = [];
...
WindowManager.prototype.toggleDragging = function (enabled) {
  windowManager.forEachWindow(function (dialog) {
    dialog.togglePointerEvents(!enabled);
  });
...
function ClickOffset() {
  ...
  this.stats = {
    ...
    update: function (/** @type {number}*/ x, /** @type {number}*/ y) {
      ...
      this.difference = (this.lastPosition = this.positions.shift())
        .clone()
        .sub(this.position);
      this;
      return this;
...
function Dialog(object, create) {
  ...
  if (!(object instanceof HTMLElement)) this.application = object;
...
Dialog.prototype.initWithObject = function (object) {
  ...
  if (object instanceof HTMLElement) {
    ...
    if (
      ...
      this.target.parentElement.nodeName === "TEMPLATE"
    )
      return;
    this.target = object;
    ...
    if (object.classes && typeof object.classes === "object")
      object.classes.forEach(function (someclass) {
        this.target && this.target.classList.add(someclass);
      }, this); // We can't use class since it's a keyword!!
    this.openUrl(object.src);
    this.setTitle(object.title);
    this.fixed = object.fixed;
    this.scroll = object.scroll;
    if (this.frame) {
      if (object.microphone || object.camera)
  ...
  this.exchangeDialogMouseUpEvent = this.messageFrame.bind(this, "mouseUp", {
    difference: new Vector()
  });
  ...
  this.exchangeDialogMoveEvent = function (difference) {
    // Async is not supported in IE11?!? I chose some async since we don't need the return value and I need the window move to be as fast as possible. The next best option is a service worker!!
    if (difference && self.clickOffset)
      this.messageFrame(
        "windowMove",
        self.clickOffset.stats.update(difference.x, difference.y)
      );
  };
  ...
  if (target && body) {
      ...
      for (var index = 0; index < 8; index++) {
        ...
        var pointerDown = function (ev) {
          if (ev.target && ev.target instanceof HTMLElement)
            dragAction.set(Number(ev.target.id));
    ...
    target.addEventListener(
      "animationend",
      function (ev) {
        if (ev.animationName == "closing") dialog.kill.apply(dialog);
      },
      false
    );
    ...
    target
      ...
      [windowButtons.eject].addEventListener("click", function (event) {
        if (!target) return;
        ...
        if (self.href)
          self._popupWindow = window.open(
            self.href,
            self.title,
            stringifyDialogProperties(propeties)
          );
        self.quit();
      });

    var buttons = target.getElementsByTagName("button");
    buttons[windowButtons.close].addEventListener(
      "click",
      function () {
        self.close();
      }.bind(self)
...
function translateElement(element, x, y) {
  element.style.transform =
    "translate(" + toPixels(x) + "," + toPixels(y) + ")";
  element.style.webkitTransform =
    "translate(" + toPixels(x) + "," + toPixels(y) + ")"; // TODO: Only do this when necessary
}
  ...
  get: function () {
    return Boolean(
      this.target &&
        ...
        bodyCrawler.getDialogsContainer()?.contains(this.target)
    );
  },
  set: function (force) {
    if (this.target)
      this.target.toggleAttribute("open", force), this.activate();
  }
});
Object.defineProperty(Dialog.prototype, "frame", {
  get: function () {
    return (
      (this.target && this.target.getElementsByTagName("iframe")[0]) ||
      (this.body && this.body.appendChild(document.createElement("iframe")))
    );
  }
});
...
Object.defineProperty(Dialog.prototype, "body", {
  get: function () {
    return this.content
      ? this.content.children[1] instanceof HTMLElement
...
Object.defineProperty(Dialog.prototype, "head", {
  get: function () {
    return this.target ? this.target.getElementsByTagName("header")[0] : null;
  }
});
...
Object.defineProperty(Dialog.prototype, "width", {
  ...
  set: function (width) {
    ...
    this.target.style.width = toPixels(
...
Object.defineProperty(Dialog.prototype, "height", {
  ...
  set: function (height) {
    if (typeof height == "number" && this.target)
      this.target.style.height = toPixels(
...
Object.defineProperty(Dialog.prototype, "title", {
  get: function () {
    return this._title || this.getTitleElement().textContent || this.id;
  },
  set: function (title) {
    this.setTitle(title);
  }
...
Object.defineProperty(Dialog.prototype, "micaElement", {
  get: function () {
    try {
      ...
      var clip =
        this.target.getElementsByClassName("backdrop-clip")[0].children[0];
...
Dialog.prototype.getTitleElement = function () {
  return this.head && this.head.querySelector("h1");
};
/** @param {boolean} force */
...
Dialog.prototype.setClickOffset = function (x, y) {
  ...
  return (
    (this.clickOffset.x = x),
    (this.clickOffset.y = y),
    (this.clickOffset.height = window.height || rect.height),
    (this.clickOffset.width = window.width || rect.width),
    (this.clickOffset.top = rect.top),
    (this.clickOffset.left = rect.left),
...
Object.defineProperty(Dialog.prototype, "href", {
  get: function () {
...
Dialog.prototype.togglePointerEvents = function (enable) {
  if (enable == null && this.target)
    enable = this.target.style.pointerEvents == "none";
...
Dialog.prototype.removeMica = function () {
  if (!this.target) return;
  var clip = this.target.getElementsByClassName("backdrop-clip")[0];
  if (clip && clip.children.length) clip.removeChild(clip.children[0]);
};
...
function DragAction() {
  ...
  this.resizeFunctions = [
    function (dialog, offset, difference) {
      dialog.move(offset.left + difference.x, offset.top + difference.y);
    }, // Move
    function (dialog, offset, difference) {
    ...
    function (dialog, offset, difference) {
      (dialog.width = offset.width + difference.x),
        (dialog.height = offset.height - difference.y),
        (dialog.y = offset.top + difference.y);
    }, // Top right
    function (dialog, offset, difference) {
      (dialog.height = offset.height + difference.y),
        (dialog.width = offset.width + difference.x);
    ...
    function (dialog, offset, difference) {
      (dialog.x = offset.left + difference.x),
        (dialog.width = offset.width - difference.x),
        (dialog.height = offset.height + difference.y);
    } // Bottom cleft?
  ];
...
DocumentCrawler.prototype = {
  ...
  }, // I don't know why I didn't use getters to start with.
  get charms() {
    return document.getElementById("charms");
  },
  get settings() {
...
function messageReceived(type, data, source) {
  // I have yet to make a wrapper function that takes care of the types and data parsing for ease of use by another user who doesn't understand what I'm doing here, it needs to be done manually by me for now!
  var types = LVMessenger.types;
  if (source) {
    var dialog = windowManager.windows[source];
    if (!dialog) {
      console.warn(
        "Received message for unknown window source:",
        source,
        type,
        data
      );
    ...
    if (type === types.windowSize && dialog)
    ...
    switch (type) {
      case types.launchOverlay:
        ...
        bodyCrawler.overlay.ontransitionend = function () {
          ...
          window.setTimeout(
            bodyCrawler.overlay.classList.add.bind(
...
function checkForFlip() {
  ...
  if (
    false &&
    window.matchMedia(
      "only screen and (max-width: 300px), (pointer:none), (pointer:coarse)"
    ).matches
  ) {
...
function initializeDialogs() {
  if (typeof onpointerup !== "undefined")
...
function handleWindowDrag(newX, hewY) {
  ...
  /** @type {Position} */
  var difference = {
    x: newX - dialog.clickOffset.x,
    y: hewY - dialog.clickOffset.y
  };

  dragAction.execute(dialog, dialog.clickOffset, difference);
  if (dialog.moveEvents && dialog.exchangeDialogMoveEvent)
    dialog.exchangeDialogMoveEvent(difference);
}
...
function toggleDialogDragEventHandler(enable) {
  if (enable)
    document.addEventListener(
      supportsPointer ? "pointermove" : "mousemove",
      windowDragEvent
    ),
...
function updateTopZ(newZ) {
  if (newZ)
    if (newZ > topZ) topZ = newZ;
    else
      windowManager.forEachWindow(function (dialog) {
        if (dialog.z > topZ) topZ = dialog.z;
      });
}
...
function getDialogBody(target) {
  // I am specifically not using querySelector in case we want an actual HTMLElement reference instead of a node! QuerySelector may be faster but I'm not using this function in time sensitive operations like the window drag, so I prefer functionality instead. The most left is the most recent revision. I removed the deprecated ones but if I make even more changes to the design of the dialogs I'll have to clean it up again or it'll get too long. We theoretically only need one, so as soon as I rebuilt all dialogs it can be simplified to one.
  if (!target) return null;
  var body =
    target.getElementsByTagName("content")[1] ||
    target.getElementsByTagName("section")[1] ||
    target.querySelector("article") ||
    target.getElementsByClassName("client")[0] ||
    target.getElementsByTagName("iframe")[0] ||
    target.getElementsByTagName("section")[1] ||
    target.getElementsByClassName("body")[0] ||
    target.children[2];
...
function handleStorageException(exception) {
  ...
  console.warn(
    "A problem occurred, window state saving has been disabled for this session! The stored window state will be reset in an attempt to recover from this issue."
  );
  console.log(
    "If you wish to save the window state before reset, copy this and put it somewhere else:",
...
Dialog.prototype.exportDialogBodyToMetro = function () {
  ...
  if (metro && this.body)
    (metroBodyOrigin = this.id), metro.appendChild(this.body);
};
...
function removeComments(element) {
  // Removes the comments of an HTMLElement based object.
